...
if ($session instanceof erLhcoreClassModel2FASession) {
    ...
    $secret = erLhcoreClassModel2FAUser::findOne(array('filter' => array('method' => 'ga', 'enabled' => 1, 'user_id' => $session->user_id)));

    if ($secret instanceof erLhcoreClassModel2FAUser) {

        $g = new \Google\Authenticator\GoogleAuthenticator();

        if ($g->checkCode($secret->attr_array['secret'], $_POST['code'])) {
