...
$tpl = erLhcoreClassTemplate::getInstance( '2fa/authenticationsetup.tpl.php');
...
if ($session instanceof erLhcoreClassModel2FASession && $session->ctime > (time()-(10*60)) && $session->retries < erLhcoreClassModel2FASession::RETRIES_MAX) {
    $tpl->set('methods', erLhcoreClassModel2FAUser::getList(array('sort' => '`default` DESC','filter' => array('enabled' => 0, 'default' => 1, 'user_id' => $session->user_id))));
